...
// GIS SITES BUILD FILTER/TABLE CONFIGURATION
...
function gisSitesBuildFilter() {
  $("#gisSites-Filter_DATA").queryBuilder({
    allow_empty: true,
    filters: filters
  });
}

function gisSitesApplyFilter() {
  var query = "SELECT * FROM ?";
  var sql = $("#gisSites-Filter_DATA").queryBuilder("getSQL", false, false).sql;
  if (sql.length > 0) {
    query += " WHERE " + sql;
  }
  alasql(query, [gisSitesData.features], function(features){
    gisSitesLayer.clearLayers();
    gisSitesLayer.addData(features);
    //syncSitesTable();
    map.flyToBounds(gisSitesLayer.getBounds());
  });
}
